...
bool ImGuiToggleRenderer::Render()
{
	...
	ImRect total_bounding_box = ImRect(widget_position,
	    widget_position
	        + ImVec2(width + (label_size.x > 0.0f ? _style->ItemInnerSpacing.x + label_size.x : 0.0f) + label_x_offset,
...
void ImGuiToggleRenderer::DrawA11yFrameOverlay(float knob_radius, bool state)
{
	...
	const ImVec2 adjustment = ImVec2(radius - thickness, 0.0f) * (state ? -1.0f : 1.0f); // if state is true, we want to subtract rather than add.
	...
	{
	case ImGuiToggleA11yStyle_Label: DrawA11yLabel(pos, color, state ? _config.On.Label : _config.Off.Label); break;
	case ImGuiToggleA11yStyle_Glyph: DrawA11yGlyph(pos, color, state, radius, thickness); break;
	case ImGuiToggleA11yStyle_Dot: DrawA11yDot(pos, color); break;
	default: break;
	}
}
...
void ImGuiToggleRenderer::UpdatePalette()
{
	...
	if (!IsAnimated())
	{
		ImGui::UnionPalette(&_palette, *_value ? on_candidate : off_candidate, _style->Colors, *_value);
...
ImVec2 ImGuiToggleRenderer::CalculateKnobCenter(float radius, float animation_percent, const ImVec2& offset /*= ImVec2()*/) const
{
	...
	const float knob_x = (pos.x + radius) + animation_percent * (GetWidth() - double_radius - offset.x)
...
ImRect ImGuiToggleRenderer::CalculateKnobBounds(float radius, float animation_percent, const ImVec2& offset /*= ImVec2()*/) const
{
	...
	const float knob_left = (animation_percent * (GetWidth() - double_radius - offset.x))
	    + (animation_percent_inverse * offset.x) + _state.KnobInset.Left;
