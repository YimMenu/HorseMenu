...
{
	static bool LoadGroundAtCoords(Vector3& coords)
	{
		float groundZ;
		bool done = false;
	...
	static bool TeleportEntity(Entity ent, Vector3& coords)
	{
	...
	static Vector3 GetWaypointCoords()
	{
	...
	class TpToWaypoint : public Command
	{
		...
		virtual void OnCall() override
		{
			if (MAP::IS_WAYPOINT_ACTIVE())
			{
				auto waypointCoords = GetWaypointCoords();
				TeleportEntity(Self::PlayerPed, waypointCoords);
			}
		}
	};
